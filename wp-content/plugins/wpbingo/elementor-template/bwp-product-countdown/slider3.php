...
if ( $list -> have_posts() ){ ?>
	<div id="<?php echo $widget_id; ?>" class="bwp-countdown <?php echo esc_attr($layout); ?> <?php echo esc_attr($class); ?> <?php if(empty($title1)) echo 'no-title'; ?>">       
		<?php if($title1) { ?>
		<div class="box-title font-extrabold">
			<?php if($subtitle) { ?>
			<div class="subtitle"><?php echo esc_html($subtitle); ?></div>
			<?php } ?> 
			<div class="title-block"><?php echo esc_html($title1); ?></div>   
		</div> 
		<?php } ?>
		<div class="product-list">	
			<div class="content-product-list">	
				...
				<?php while($list->have_posts()): $list->the_post();?>
					...
					$product_hot_label = isset($davici_settings['product-hot-label']) && !empty($davici_settings['product-hot-label']) ? $davici_settings['product-hot-label'] : esc_html__('Hot','wpbingo');
					$product_sale_label = davici_get_product_discount();
					$product_sale = davici_get_config('product-sale',true);
							...
							<div class="grid-content">
								...
								<div class="products-content">
									...
									<div class="item-countdown">
										<h2><?php echo esc_html__("end in :","wpbingo") ?></h2>
